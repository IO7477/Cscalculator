...
const detectExpressionType = (expression: string): 'infix' | 'prefix' | 'postfix' | 'unknown' => {
  ...
  // Check for operators between operands (infix pattern)
  for (let i = 1; i < tokens.length - 1; i++) {
    if (isOperator(tokens[i]) && isOperand(tokens[i - 1]) && isOperand(tokens[i + 1])) {
      return 'infix';
    }
  }

  // Prefix: operator first
  if (hasOperatorBeforeOperands && !hasOperatorAfterOperands) return 'prefix';
  
  // Postfix: operator last
  if (hasOperatorAfterOperands && hasOperandBeforeOperator) return 'postfix';

  // Check sequence: if mostly "operand operand operator", it's postfix
  let postfixScore = 0;
  let prefixScore = 0;
  for (let i = 0; i < tokens.length - 2; i++) {
    if (isOperand(tokens[i]) && isOperand(tokens[i + 1]) && isOperator(tokens[i + 2])) {
      postfixScore++;
    }
    if (isOperator(tokens[i]) && isOperand(tokens[i + 1]) && isOperand(tokens[i + 2])) {
      prefixScore++;
    }
  }

  if (postfixScore > prefixScore) return 'postfix';
  if (prefixScore > postfixScore) return 'prefix';
